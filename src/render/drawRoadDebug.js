...
export function drawRoadDebug(r, state){
  ...
  /* @tweakable debug arrow length multiplier */
  const arrowLengthMultiplier = 1.0;
  /* @tweakable debug arrow head size */
  ...
  for (let y=0; y<hTiles; y++) for (let x=0; x<wTiles; x++){
    ...
    if (outs.length === 2) {
      ...
      drawAngleArrow(ctx, cxp, cyp, ang, ts*0.28 * arrowLengthMultiplier);
    } else if (outs.length >= 1) {
      const v = outs[0]; const ang = Math.atan2(v.y - gy, v.x - gx);
      drawAngleArrow(ctx, cxp, cyp, ang, ts*0.28 * arrowLengthMultiplier);
    } else {
      drawDirArrow(ctx, cxp, cyp, d, ts*0.28 * arrowLengthMultiplier);
    }
  }
  ctx.restore();
}
...
function drawAngleArrow(ctx, cx, cy, ang, len){
  ...
  const ah = arrowHeadSize, a1 = ang + 2.6, a2 = ang - 2.6;
