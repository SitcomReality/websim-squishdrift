...
export class GameEngine {
  ...
  render(interp) {
    ...
    // Draw skidmarks after ground tiles but before buildings
    const ts = this.state.world.tileSize;
    const { ctx } = this.renderer;
    const cx = Math.floor(this.renderer.canvas.width / 2);
    const cy = Math.floor(this.renderer.canvas.height / 2);
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ctx.translate(cx, cy);
    ctx.scale(this.state.camera.zoom || 1, this.state.camera.zoom || 1);
    ctx.translate(Math.floor(-this.state.camera.x * ts), Math.floor(-this.state.camera.y * ts));
    
    this.systems.skidmark.render(this.renderer, this.state);
