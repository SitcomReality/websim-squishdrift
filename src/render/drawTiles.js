...
function drawZebraCrossing(r, gx, gy, ts, tileType, map) {
  ...
  switch(tileType) {
    ...
    case Tile.ZebraCrossingS:
      ...
      for (let i = 0; i < 5; i++) {
        const x = gx*ts + centerOffset - (2.5 * stripeWidth) + (i * (stripeWidth + gapWidth));
        if (x + stripeWidth <= (gx+1)*ts && x >= gx*ts) {
          ctx.fillRect(x, gy*ts, stripeWidth * 2, ts); // Double width for central line
        }
      }
      break;
    ...
    case Tile.ZebraCrossingW:
      ...
      for (let i = 0; i < 5; i++) {
        const y = gy*ts + centerOffset - (2.5 * stripeWidth) + (i * (stripeWidth + gapWidth));
        if (y + stripeWidth <= (gy+1)*ts && y >= gy*ts) {
          ctx.fillRect(gx*ts, y, ts, stripeWidth * 2); // Double height for central line
        }
      }
      break;
  }
