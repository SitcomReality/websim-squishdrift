export class AudioManager {
  ...
  playSfxAt(key, pos, state, { volume = this.sfxVolume, minDistance = 2, maxDistance = 18, panMax = 12, startOffset = 0 } = {}) {
    if (this.sfxMuted) return;
    const buf = this.buffers.get(key);
    ...
    // Apply random start offset if provided
    if (startOffset > 0) {
    ...
    // Spatial attenuation and pan (only if not just starting)
    if (startOffset <= 0) {
      const t = this.ctx.currentTime;
      try {
        node.src.playbackRate.cancelScheduledValues(t);
        node.src.playbackRate.linearRampToValueAtTime(Math.max(0.01, 1), t + 0.05);
      } catch {
        node.src.playbackRate.value = Math.max(0.01, 1);
      }
      try {
