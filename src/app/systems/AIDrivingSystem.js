  ...
  updateMovement(v, dt) {
      ...
      if (!v.retreatState || !v.retreatState.active) {
        ...
        if (v.stuckTimer > STUCK_THRESHOLD) {
          ...
          v.retreatState = {
            ...
            giveUpTimer: 0 // timer to give up if no backward progress
          };
    ...
    if (v.retreatState && v.retreatState.active) {
      ...
      if (backwardComponent > 0) {
        ...
        // reset give-up timer when we actually make backward progress
        v.retreatState.giveUpTimer = 0;
      } else {
        // If we're not making backward progress, increment give-up timer
        v.retreatState.giveUpTimer = (v.retreatState.giveUpTimer || 0) + dt;
        const GIVE_UP_THRESHOLD = 0.7; // seconds to give up trying to reverse
        if (v.retreatState.giveUpTimer >= GIVE_UP_THRESHOLD) {
          // Give up on reversing — cancel retreat and allow normal forward pathing
