    ...
    for (const v of state.entities.filter(e => e.type === 'vehicle')) {
      ...
      if ((drifting || hardBrake) && speed > 0.05) {
        ...
        if (moved >= this.skidMinSegmentSpacing) {
          // Start continuous tire skid loop with random offset
          if (!v._isSkidding) {
            const randomOffset = Math.random() * 4; // 0-4 seconds offset
            state.audio?.startOrUpdateLoopAt(
              'tire_skid_loop', 
              v, 
              v.pos, 
              state,
        ...
        // If not skidding/braking, stop the continuous skid sound
        if (v._isSkidding) {
          state.audio?.stopLoop(v, { fadeOut: 0.3 });
          v._isSkidding = false;
        }
        
        // Reset the last position and blood state
